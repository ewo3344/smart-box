...
| 9 | NODE_SCORE_FAILURE_PENALTY | FAIL | 2026-08-30 | 关 Wi-Fi/数据后对基准 Smart 点「测试」，组页仍只显示 urlTestDelay（全局 401ms、新加坡/AI Fallback 544ms），未见 +500 失败罚分数字。代码确认（2026-08-30）：组页「测试」经 `StartedService.URLTest` → `Smart.CheckOutbounds` → `probe(force=true)` → `applySmartProbeState`；测速失败会 `state.failures++` 并 `queueScore`。组页只渲染 `urlTestDelay`。属实现缺陷，计划 v0.1.2 |
...
- **1/15 FAIL**：第 9 项（失败罚分）— 手动 urlTest 会写入 `failures` 并更新 score，组页只显示 `urlTestDelay`，未见 +500
- **2/15 DEFERRED**：
...
- 节点分数机制（2/3 项，第 9 项 FAIL）
- 网络切换、Telegram 收发（2 项）通过
- 系统权限（2 项）通过

未验证项（3 项）均有明确理由：

- 第 9 项：FAIL（手动 urlTest 写 `failures`，组页不显示罚分；v0.1.2）
- 第 10 项：时间窗口限制（7 天衰减）
- 第 12 项：隐私保护（抖音评论）
