...
```bash
cd converter
env GOTOOLCHAIN=go1.25.5 go test ./...
env GOTOOLCHAIN=go1.25.5 go test -count=50 ./...
env GOTOOLCHAIN=go1.25.5 go test -race ./...
env GOTOOLCHAIN=go1.25.5 go vet ./...
env SMART_BOX_CORE=/tmp/smart-box-core-check GOTOOLCHAIN=go1.25.5 \
  go test -run 'TestGeneratedProfile(AcceptedByCore|StartsWithRemoteRuleSets)' -v .
env CGO_ENABLED=0 GOOS=linux GOARCH=arm64 GOTOOLCHAIN=go1.25.5 \
...
env SMART_BOX_LIVE_RULESETS=1 SMART_BOX_CORE=/tmp/smart-box-core-check \
  GOTOOLCHAIN=go1.25.5 go test -run TestLiveRuleSetSourcesAcceptedByCore -v .
```
